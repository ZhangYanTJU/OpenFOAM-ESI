...
)
{
    return
        ...
        (
            ...
            mesh.time().timeName(),
            mesh,
            mustRead ? IOobject::MUST_READ : IOobject::READ_IF_PRESENT,
            IOobject::NO_WRITE,
...
{
    tmp<volScalarField::Internal> tinvLsqr = scalar(1)/sqr(L);
    const volScalarField::Internal& invLsqr = tinvLsqr.cref();
    ...
    // (M:p. 2)
    const dimensionedScalar fMax
    (
        dimless,
        scalar(1) - Foam::exp(-scalar(400)/scalar(50))
    );

    // Heuristic
    const dimensionedScalar fMin(dimless, Zero);
...
int main(int argc, char *argv[])
{
    ...
    const bool initEpsilon = dict.getOrDefault<bool>("initialiseEpsilon", false);
    ...
    (
        createIOobject(mesh, UName, mustRead),
        mesh
    );
    ...
    // (M:p. 2)
    tmp<volScalarField::Internal> tL = scalar(50)*nu.v()/uTau;
    const volScalarField::Internal& L = tL.cref();

    calcF(L, f);

    // (M:Eq. 8)
    tmp<volScalarField::Internal> td = -tL*Foam::log(scalar(1) - f.v());
    const volScalarField::Internal& d = td.cref();

    // (M:p. 2)
    const volScalarField::Internal dPlus(d*uTau/nu.v());

    // (M:Eq. 11)
    const volScalarField::Internal epsilon
    (
        pow4(uTau)/(kappa*nu.v()*max(dPlus, dPlusRef))
    );

    // (M:Eq. 13)
    const volScalarField::Internal fk(Foam::exp(-dPlus/scalar(25)));

    // (M:Eq. 12)
    const volScalarField::Internal k
    (
        (epsilon*sqr(td)*sqr(fk))/(2*nu.v())
    ...
    if (initU)
    {
        volVectorField::Internal& U = tU.ref().ref();

        // Reichardt’s law (M:Eq. 10)
        const scalar C = 7.8;
        const scalar B1 = 11;
        const scalar B2 = 3;
        const volScalarField::Internal fRei
        (
        ...
        tU->boundaryFieldRef().evaluateCoupled<coupledFvPatch>();
    }

    if (tepsilon.valid())
    {
        tepsilon.ref().ref() = epsilon;
        tepsilon->boundaryFieldRef().evaluateCoupled<coupledFvPatch>();
    }

    if (tk.valid())
    {
        tk.ref().ref() = k;
        tk->boundaryFieldRef().evaluateCoupled<coupledFvPatch>();
    }

    if (tomega.valid())
    {
        const dimensionedScalar k0(sqr(dimLength/dimTime), SMALL);
        tomega.ref().ref() = Cmu*epsilon/(k + k0);
        tomega->boundaryFieldRef().evaluateCoupled<coupledFvPatch>();
    }

    if (tR.valid())
    {
        volSymmTensorField::Internal& R = tR.ref().ref();

        // (M:Eq. 3)
        const volSphericalTensorField::Internal Rdiag(k*twoThirdsI);
        forAll(R, celli)
        {
            R[celli] = Rdiag[celli];
        }
        tR->boundaryFieldRef().evaluateCoupled<coupledFvPatch>();
    }
