...
void Foam::solarCalculator::initialise()
{
    ...
    switch (sunLoadModel_)
    {
        ...
        case mSunLoadTimeDependent:
        {
            directLoads_.reset
            (
                new TimeFunction1<scalar>(mesh_.time(), "directLoads", dict_)
            );

            diffuseLoads_.reset
            (
                new TimeFunction1<scalar>(mesh_.time(), "diffuseLoads", dict_)
            );

            directSolarRad_ = directLoads_->value(mesh_.time().value());
            diffuseSolarRad_ = diffuseLoads_->value(mesh_.time().value());
            break;
        }
