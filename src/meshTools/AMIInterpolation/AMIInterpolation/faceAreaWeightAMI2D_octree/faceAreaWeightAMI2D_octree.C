...
#include "faceAreaWeightAMI2D_octree.H"
#include "profiling.H"
#include "OBJstream.H"
...
namespace Foam
{
    defineTypeNameAndDebug(faceAreaWeightAMI2D_octree, 0);
    addToRunTimeSelectionTable(AMIInterpolation, faceAreaWeightAMI2D_octree, dict);
    addToRunTimeSelectionTable(AMIInterpolation, faceAreaWeightAMI2D_octree, component);
}

// * * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * //

void Foam::faceAreaWeightAMI2D_octree::writeNoMatch
(
...
void Foam::faceAreaWeightAMI2D_octree::storeInterArea
(
    const label srcFacei,
...
) const
{
    ...
    for (const auto& tris : srcTris)
    {
        ...
        vector axis1(p10/mag(p10));
        vector axis2(srcFaceNormals[srcFacei]^axis1);
...
Foam::faceAreaWeightAMI2D_octree::faceAreaWeightAMI2D_octree
(
...
:
    ...
    Cbb_(dict.getOrDefault<scalar>("Cbb", 0.1))
{}
...
Foam::faceAreaWeightAMI2D_octree::faceAreaWeightAMI2D_octree
(
    const faceAreaWeightAMI2D_octree& ami
)
:
...
bool Foam::faceAreaWeightAMI2D_octree::calculate
(
...
)
{
    ...
    addProfiling(ami, "faceAreaWeightAMI2D_octree::calculate");
    ...
    if (validSize)
    {
        ...
        forAll(src, srcFacei)
        {
            ...
            const labelList tgtFaces = tree.findBox(srcFaceBb);
            ...
            if (mustMatchFaces() && srcAddr.empty())
            {
                ...
                // Try find nearest...
                label tgtFacei = findTargetFace(srcFacei);
...
void Foam::faceAreaWeightAMI2D_octree::write(Ostream& os) const
{
