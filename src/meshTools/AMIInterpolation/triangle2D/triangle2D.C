...
Foam::label Foam::triangle2D::snapClosePoints(const triangle2D& triB)
{
    ...
    for (auto& a : triA)
    {
        forAll(triB, tb)
        {
            if (match[tb] && sameClosePoint(a, triB[tb]))
            {
                a = triB[tb];
                match[tb] = false;
                ++nSnapped;
...
bool Foam::triangle2D::intersects(const triangle2D& triB) const
{
    const triangle2D& triA = *this;
    ...
    return
        (nSamePoints(triA, triB) == 3) // same tri
