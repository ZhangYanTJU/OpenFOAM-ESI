...
{
    const label facei = faceLabel(index);
...
Foam::tmp<Foam::pointField> Foam::treeDataFace::shapePoints() const
{
    if (useSubset_)
    {
...
) const
{
    ...
    const face& f = mesh().faces()[faceLabel(index)];
...
) const
{
    ...
    const label facei = shape.faceLabel(index);
