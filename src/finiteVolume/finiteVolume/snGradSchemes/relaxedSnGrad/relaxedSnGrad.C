...
) const
{
    ...
    tmp<SType> tcorrection = correctedScheme_().correction(vf);
    ...
    if (!obr.foundObject<SType>(oldFieldName))
    {
        SType* oldCorrection = new SType(oldFieldName, tcorrection());
        oldCorrection->store();
    }

    // Return under/over-relaxed explicit correction field
    tmp<SType> trelaxedCorrection(new SType(tcorrection()));

    SType& oldCorrection = obr.lookupObjectRef<SType>(oldFieldName);
