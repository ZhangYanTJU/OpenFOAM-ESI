...
) const
{
    ...
    typedef GeometricField<GradType, fvPatchField, volMesh> VolGradType;
    typedef GeometricField<GradType, fvsPatchField, surfaceMesh> SurfGradType;
    ...
    tmp<VolGradType> tgGrad = fv::gaussGrad<Type>::gradf(ssf, name);
    VolGradType& gGrad = tgGrad.ref();

    const skewCorrectionVectors& skv = skewCorrectionVectors::New(vsf.mesh());

    for (label i = 0; i < nIter_; ++i)
    {
        tmp<SurfGradType> tsgGrad = linearInterpolate(gGrad);
...
template<class Type>
void Foam::fv::iterativeGaussGrad<Type>::correctBoundaryConditions
(
    const GeometricField<Type, fvPatchField, volMesh>& vsf,
    GeometricField
    <
