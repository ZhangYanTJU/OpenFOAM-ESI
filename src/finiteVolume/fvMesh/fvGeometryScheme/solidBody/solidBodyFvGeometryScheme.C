...
void Foam::solidBodyFvGeometryScheme::setMeshMotionData()
{
    if (!cacheInitialised_ || !cacheMotion_)
    {
        ...
        for (label patchi = 0; patchi < pbm.size(); ++patchi)
        {
            ...
            const auto& changedPatchFaceFlag = pp.patchSlice(changedFaceFlag);
            forAll(changedPatchFaceFlag, patchFacei)
            {
                if (changedPatchFaceFlag[patchFacei])
                {
                    changedFaceIDs.append(patchFacei + pp.start());
