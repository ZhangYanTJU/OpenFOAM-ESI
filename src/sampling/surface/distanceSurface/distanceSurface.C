...
// Absolute distance from surface hit point -> point in the mesh
static inline scalar absoluteDistance
(
    const point& pt,
    const pointIndexHit& pHit
)
{
    return Foam::mag(pt - pHit.point());
}
...
)
{
    forAll(nearest, i)
    {
        distance[i] = absoluteDistance(points[i], nearest[i]);
    }
}
...
void Foam::distanceSurface::createGeometry()
{
    ...
    const searchableSurface& geom = surfacePtr_();
    ...
    {
        if (topologyFilterType::NONE != topoFilter_)
        {
            ...
            // Refine the blocked cells by checking the actual cells cut
