...
// narrowing(readType val, dataType bigValue, dataType smallValue) noexcept
// {
//     return
//     (
//         (val <= -bigValue) ? -bigValue
//       : (val >=  bigValue) ?  bigValue
//       : (val > -smallValue && val < smallValue) ? 0
//       : dataType(val)
//     );
// }
    ...
    else
    {
        for (const dataType* last = data + nElem; data != last; ++data)
        {
            ...
            if constexpr (sizeof(dataType) < sizeof(readType))
            {
                ...
                //
                // NB: not fully general - it uses VSMALL/VGREAT
                // use pTraits<dataType>::(min|max|vsmall) instead?

                *data =
                (
                    (val <= -Foam::VGREAT) ? -Foam::VGREAT
                  : (val >=  Foam::VGREAT) ?  Foam::VGREAT
                  : (val > -Foam::VSMALL && val < Foam::VSMALL) ? 0
                  : dataType(val)
                );
            }
            else
