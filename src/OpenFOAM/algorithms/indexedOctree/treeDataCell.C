...
)
{
    ...
    else
    {
        for (const label celli : cellIds)
        {
            bb.add(treeBoundBox(mesh.cells()[celli].box(mesh)));
        }
    }
...
{
    const label celli = cellLabel(index);

    if (mesh_.hasCellPoints())
    {
        return treeBoundBox(mesh_.points(), mesh_.cellPoints(celli));
    }
    return treeBoundBox(mesh_.cells()[celli].box(mesh_));
}
...
Foam::tmp<Foam::pointField> Foam::treeDataCell::shapePoints() const
{
    if (useSubset_)
    {
...
{
    return mesh_.pointInCell(sample, cellLabel(index), decompMode_);
}
...
) const
{
    ...
    const label celli = shape.cellLabel(index);
