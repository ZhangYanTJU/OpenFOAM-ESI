...
{
    const auto& model = time_.objectRegistry::lookupObject<filmType>(filmName_);

    const fvMesh& filmMesh = model.regionMesh();

    surfaceScalarField* resultPtr =
        filmMesh.lookupObjectRefPtr<surfaceScalarField>(resultName_);
    ...
    const volScalarField::Internal& magSf = model.magSf()();
...
{
    const auto& model = time_.objectRegistry::lookupObject<filmType>(filmName_);
    model.regionMesh().lookupObject<surfaceScalarField>(resultName_).write();
