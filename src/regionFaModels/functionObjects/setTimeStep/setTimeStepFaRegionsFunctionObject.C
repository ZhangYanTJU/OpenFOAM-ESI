...
Foam::scalar
Foam::functionObjects::setTimeStepFaRegionsFunctionObject::regionDeltaT() const
{
    wordList names = time_.sortedNames<regionFaModel>();

    scalar Co(0.0);

    forAll (names, i)
    {
        const regionFaModel* regionFa =
            time_.cfindObject<regionFaModel>(names[i]);
        if (regionFa)
        {
            scalar regionCo = regionFa->CourantNumber();
            if (regionCo > Co)
            {
                Co = regionCo;
            }
    ...
    if (names.size() > 0)
    {
        scalar regionFaMaxCo =
            time_.controlDict().get<scalar>("regionFaMaxCo");

        scalar maxDeltaTFact = regionFaMaxCo/(Co + SMALL);
        scalar deltaTFact =
